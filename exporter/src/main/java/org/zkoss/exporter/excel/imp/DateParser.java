...
{{IS_NOTE
	...
	History:
		Nov 14, 2012 11:24:54 AM , Created by Sam
}}IS_NOTE
...
public class DateParser {
	...
	public DateParser(Locale locale) {
		...
		_formatInfos.add(new FormatInfo(Pattern.compile(datetime11[0], flags), "m/d/yyyy h:mm", datetime11[1], datetime11[2], locale));
		
		final String[] datetime12 = getDate12(locale, true); 
		_formatInfos.add(new FormatInfo(Pattern.compile(datetime12[0], flags), "m/d/yyyy h:mm", datetime12[1], datetime12[2], locale));
		
		final String[] datetime21 = getDate21(locale, true); 
		_formatInfos.add(new FormatInfo(Pattern.compile(datetime21[0], flags), "m/d/yyyy h:mm", datetime21[1], datetime21[2], locale));
		
		final String[] datetime22 = getDate22(locale, true); 
		_formatInfos.add(new FormatInfo(Pattern.compile(datetime22[0], flags), "m/d/yyyy h:mm", datetime22[1], datetime22[2], locale));
		
		final String[] datetime31 = getDate31(locale, true); 
		_formatInfos.add(new FormatInfo(Pattern.compile(datetime31[0], flags), "m/d/yyyy h:mm", datetime31[1], datetime31[2], locale));
		
		final String[] datetime32 = getDate32(locale, true); 
		_formatInfos.add(new FormatInfo(Pattern.compile(datetime32[0], flags), "m/d/yyyy h:mm", datetime32[1], datetime32[2], locale));
		
		final String[] datetime4 = getDate4(locale, true); 
		_formatInfos.add(new FormatInfo(Pattern.compile(datetime4[0], flags), "m/d/yyyy h:mm", datetime4[1], datetime4[2], locale));
	}
	
	/**
	 * Returns the date if string can parse to date, otherwise throw {@link ParseDateException}
	 * @return
